...
class FEMDomeSensor:
    def __init__(self, dt=5e-5, sub_steps = 50,  init_img_path=None):
        ...
        self.E_init[None], self.nu_init[None] = 0.22e4, 0.46#0.3e4, 0.445  # Young's modulus and Poisson's ratio
        ...
        self.control_vel = ti.Vector.field(3, float, shape = (self.sub_steps, self.n_verts), needs_grad=True)
    ...
    @ti.kernel
    def set_control_vel(self, f:ti.i32, finger_new :ti.types.matrix(4,4,ti.f32), finger_old :ti.types.matrix(4,4,ti.f32)):
        # rot represents rotation in local coordinate

        for i in range(self.n_verts):
            # init_t_pos = self.virtual_pos[f, i]
            init_x = ti.Vector([self.init_x[i][0], self.init_x[i][1], self.init_x[i][2], 1.0])
    ...
    def update2(self, f:ti.i32):
        for i in range(self.n_verts):
            ...
            self.vel[f+1, i] = v_out + (self.control_vel[f+1, i] - self.control_vel[f, i])
            self.pos[f+1, i] = self.pos[f, i] + self.dt * v_out

            self.virtual_pos[f+1, i] = self.virtual_pos[f, i] + self.dt * self.control_vel[f, i]
